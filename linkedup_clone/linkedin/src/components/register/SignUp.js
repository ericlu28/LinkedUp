...
const SignUp = ({ toggleModal }) => {
  ...
  const signup = async () => {
    try {
      const { about, fullname, email, password, confirmPassword, job } =
        getInputs();
      if (
        isSignupValid({
          about,
        ...
        const id = uuidv4();
        const createdAccount = { id, fullname, email, about, job };
  ...
  const getInputs = () => {
    ...
    return { about, fullname, email, password, confirmPassword, job };
  };
  ...
  const isSignupValid = ({
    ...
    if (validator.isEmpty(job)) {
      uiService.alert("Please input your job");
      return false;
    }
    ...
    <div className="signup">
      <div className="signup__content">
        ...
        <div className="signup__form">
          ...
          />
          <input type="text" placeholder="Job" ref={jobRef} />
          <textarea
